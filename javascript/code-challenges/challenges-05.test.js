...
// const toLastNames = people => people.map(e => e.firstName + ' ' + e.lastName);

// const toLastNames = people => {
  // let first = people.map(a => a.firstName);
  // let last = people.map(b => b.lastName);
  // let full = [first, last];
  // let fullName = [];
  // for(let i=0; i < full [0].length, i++){
    // fullName.push(`${fullName[0] [i]} ${fullName[1][i]}`);
  // };
  //
  // return fullName;
  // Solution code here...
...
// const addPurchases = (arr) => arr.reduce((acc, totalValue) => acc += totalValue.purchasePrice, 0);
