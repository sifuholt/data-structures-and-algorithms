...
function transformToLis(obj) {
  ...
  newArr = newArr.map((entry) => entry.join(': ')):
  return newArr.map(entry => `<li>${entry}</li>`);
};
  ...
  input.forEach(a => {
    ...
    a.forEach(element => {
      if (element % 5 === 0 && Number.isInteger(element)) {
        insideArr.push(Math.pow(2, element));
...
//
    // return innerArr.filter(element => typeof element === 'number' && element % 5 === 0)
